...
### 8.1 Kết luận
- Pipeline hoàn chỉnh: DICOM → PNG → Preprocess → Train → Evaluate
- Model đạt F1=0.79, AUC=0.81 trên RICORD single-source
- Overfitting là vấn đề chính do dataset nhỏ

### 8.2 Hướng cải thiện

#### Cải thiện mô hình
| Strategy | Mô tả | Expected Impact |
|----------|-------|-----------------|
| Giảm model size | EfficientNet-B3/B5 thay B7 | Ít overfitting |
| Transfer learning | Freeze backbone 5 epochs đầu | Giữ pretrained features |
| Cosine LR scheduler | LR giảm dần | Stabilize training |

#### Cải thiện regularization
| Strategy | Mô tả | Expected Impact |
|----------|-------|-----------------|
| Weight decay | 5e-4 → 1e-3 | Giảm overfitting |
| Dropout | 0.3 → 0.5 | Giảm overfitting |
| Label smoothing | 0.1 | Generalize tốt hơn |

#### Cải thiện data
| Strategy | Mô tả | Expected Impact |
|----------|-------|-----------------|
| Stronger augmentation | CutMix, MixUp | Generalize tốt hơn |
| Cross-validation | 5-fold | Đánh giá robust hơn |
| Data balancing | Oversampling COVID | Cải thiện sensitivity |

### 8.3 Files quan trọng

```
├── checkpoints/best_gamma0.0.pth    # Best model weights
├── train_log.txt                    # Training output
├── src/                             # Core library
├── scripts/                         # Data processing scripts
├── configurations/default.yaml      # Training config
...
- **GPU**: NVIDIA (4GB VRAM)
- **Dependencies**: timm, albumentations, scipy, opencv-python, pydicom, pandas

### 9.2 Thời gian训练
- ~10 phút/epoch (train)
- ~41 giây/epoch (validation)
- Tổng: ~3.5 giờ cho 20 epochs

### 9.3 Repo
- **GitHub**: https://github.com/BaoNgo355/multisource-covid-ct
- **Branch**: main
- **Latest commit**: "Adapt for RICORD single-source: DICOM pipeline, CSV splits, EfficientNet-B7 training"
